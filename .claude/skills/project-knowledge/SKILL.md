...
│   │   │   ├── SyntheticMarkers.cs      # Internal static: Prefixes[] = 10 strings ("[PLANNER] ", "[Plan]", "[PlanStep ", "[Tool result for step ", "[Tool Result for ", "[Executing tool: ", "[DoomLoop]", "[VerificationWarning]", "[PermissionDenied]", "[FidelityWarning]") + IsSynthetic(string?). Consumed by PlannerContextBuilder + AgentService.RunBackgroundExtraction. Public consts (AC-H7): VerificationWarningMarker = "[VerificationWarning]"; VerificationWarningPrefix = VerificationWarningMarker + " " (index 7 of Prefixes uses the const; AgentService StartsWith checks + decoration site consume these consts). Sprint 10 AC-5 consts: PermissionDeniedMarker = "[PermissionDenied]"; PermissionDeniedPrefix = "[PermissionDenied] " (index 8 of Prefixes; consumed by SummaryFailureAnalyzer + AgentService gate return). Sprint 10 Layer 4 consts: FidelityWarningMarker = "[FidelityWarning]"; FidelityWarningPrefix = "[FidelityWarning] " (index 9 of Prefixes; consumed by SummaryFailureAnalyzer + AgentService Layer 4 sentinel injection).
...
│   │   │   ├── SummaryFailureAnalyzer.cs # Internal static. Sealed record Findings(int VerificationWarnings, int RetriesExhausted, int ToolErrors, int PermissionDenials, int DoomLoops, int StepsSkippedNoToolMatch, int FidelityWarnings, IReadOnlyList<string> ExcerptedReasons) with bool HasFailures computed property (any count > 0 including FidelityWarnings). Methods: Analyze(IReadOnlyList<ConversationMessage>?) (null-safe → empty Findings; single-pass over history with mutually-exclusive else-if sentinel detection using SyntheticMarkers consts; caps ExcerptedReasons at last 3 assistant-role excerpts truncated to 200 chars). Sprint 10 follow-up Layer 3: 6th category StepsSkippedNoToolMatch detects "[PlanStep " AND "No tool matched" AND "skipping" pattern. Sprint 10 Layer 4: 7th category FidelityWarnings detects SyntheticMarkers.FidelityWarningMarker injected by AgentService Layer 4 exhaustion. BuildGroundingMessage(Findings) returns "" when !HasFailures; otherwise builds "[PlanResult]" block with non-zero counts only ("Fidelity failures: N" line for new category) and reason excerpts; injected as Role="user" message before summarize prompt. Uses SyntheticMarkers.PermissionDeniedMarker + FidelityWarningMarker.
...
│   │   │   ├── OutputFidelityVerifier.cs # Layer 4 — Output Fidelity Verifier. Public sealed class, Singleton-safe. Co-locates FidelityResult sealed record (HybridScore, SubstringScore, EmbeddingScore, bool Passed, string Reason). Constructor: (NexusConfig config, ILogger<OutputFidelityVerifier>? logger = null, IEmbeddingService? embeddings = null). VerifyAsync(IReadOnlyList<string> readToolResults, string llmSummary, CancellationToken ct) → FidelityResult?. Returns null when: disabled (OutputFidelityVerificationEnabled=false), empty readToolResults, combinedToolText.Length < 50, llmSummary.Length < 50, or outer exception (non-OCE). OCE propagates. Hybrid scoring: substringScore = SubstringScoreOf (sliding 3-gram match, benefit-of-doubt 1.0f when ngrams.Count < 3) * OutputFidelitySubstringWeight + embeddingScore * OutputFidelityEmbeddingWeight. EmbeddingScoreOfAsync: null service → 1.0f fallback; exception → log Warning + 1.0f fallback. Static helpers: Normalize (lowercase + strip [#*`~]+ + collapse whitespace), Tokenize (split on 24 delimiters, filter stopwords + len<2), Build3Grams (sliding i..i+2), CosineSimilarity (local copy; mirrors ToolPlanner.cs to avoid cross-layer cycle). 19-entry Stopwords HashSet OrdinalIgnoreCase. Logging: LogWarning with "[FidelityVerifier] attempt {Attempt}/{Max} failed…" in AgentService wire-in; "[FidelityVerifier] threw — returning null" on outer exception; "[FidelityVerifier] embedding scoring threw — falling back to 1.0f" on EmbeddingScoreOfAsync exception.
...
│   │   │   ├── ConfigLoader.cs
│   │   │   ├── NexusConfig.cs
│   │   │   └── ConfigValidator.cs    # Static validation: Memory + Models + MCP config (scalar ranges, McpServerEntry transport/url). Sprint 10 additions: validates PlannerHeuristicMinLength [1,200], PathValidatorStrictDistance [50,100]; walks Permission.Tools dict and validates Action + Patterns values are in {allow|ask|deny} (case-insensitive); error message "PermissionToolRule.Action must be 'allow', 'ask', or 'deny' (got '{action}').". Sprint 10 Layer 4 additions: ValidateOutputFidelityMinScore (range 0.0..0.95; "OutputFidelityMinScore must be between 0.0 and 0.95."), ValidateOutputFidelityWeight (range 0.0..1.0 per field; "OutputFidelity{fieldName} must be between 0.0 and 1.0."), ValidateOutputFidelityWeightSum (sum 1.0 ±0.01; "OutputFidelity weights must sum to 1.0 ± 0.01 (got {sum:F2})."), ValidateOutputFidelityMaxRetries (range 0..3; "OutputFidelityMaxRetries must be between 0 and 3.").
│   │   └── ServiceCollectionExtensions.cs # DI registration (stays at root). Sprint 10 AC-5: AgentService factory now resolves permissionGate: sp.GetService<IPermissionGate>() and verificationCatalog: sp.GetService<IVerificationCatalog>() (both optional via GetService<>). Sprint 10 Layer 4: registers services.AddSingleton<OutputFidelityVerifier>(sp => new OutputFidelityVerifier(config, sp.GetService<ILogger<OutputFidelityVerifier>>(), sp.GetService<IEmbeddingService>())); AgentService factory receives outputFidelityVerifier: sp.GetService<OutputFidelityVerifier>() as 13th optional arg.
│   │
...
// Sprint 10 Layer 4 (ServiceCollectionExtensions.cs):
services.AddSingleton<OutputFidelityVerifier>(sp => new OutputFidelityVerifier(
    config,
    sp.GetService<ILogger<OutputFidelityVerifier>>(),
    sp.GetService<IEmbeddingService>())); // 13th optional arg in AgentService factory
```
